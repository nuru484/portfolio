# Personal Portfolio Website

## 🌟 Overview

This is my personal portfolio website built using React, showcasing my professional work, skills, and experience. I designed this site to provide a comprehensive and interactive view of my professional journey.

## 🚀 Technologies Used

- React
- React Router
- State management library: Context API
- Styling framework: Tailwind CSS, Material-UI
- Animation libraries: Framer Motion

## ✨ Features

- Responsive design
- Interactive project showcases
- Detailed about me page
- Contact page

## 🛠 Installation

### Prerequisites

- Node.js (v14.0.0 or later)
- npm or Yarn

### Setup Steps

1. Clone the repository

   ```bash
   git clone git@github.com:nuru484/portfolio.git
   ```

2. Navigate to the project directory

   ```bash
   cd portfolio
   ```

3. Install dependencies

   ```bash
   npm install
   # or
   yarn install
   ```

4. Start the development server
   ```bash
   npm run dev
   # or
   yarn dev
   ```

## 🌐 Deployment

Deployed on Vercel:

- Automatic deployments from main branch

**Deployment Link**: https://portfolio-nurus-projects-d98f949e.vercel.app

## 📂 Project Structure

```
📦portfolio
 ┣
 ┃ ┗ 📜index
 ┣ 📂public
 ┃ ┣ 📂projectImages
 ┃ ┃ ┣ 📜blog site desktop.jpg
 ┃ ┃ ┗ 📜blog site phone.jpg
 ┃ ┣ 📜PostgreSQL logo.png
 ┃ ┣ 📜Tailwind_CSS_Logo.svg.png
 ┃ ┣ 📜expressjs logo.png
 ┃ ┣ 📜nodejs logo.jpg
 ┃ ┣ 📜nutrition app desktop.jpg
 ┃ ┣ 📜nutrition app mobile.jpg
 ┃ ┣ 📜profile.jpg
 ┃ ┣ 📜react logo.png
 ┃ ┣ 📜user.png
 ┃ ┣ 📜voitng system mobile.jpg
 ┃ ┗ 📜voting system desktop.jpg
 ┣ 📂src
 ┃ ┣ 📂assets
 ┃ ┃ ┗ 📜logo.jpg
 ┃ ┣ 📂components
 ┃ ┃ ┣ 📜AboutMe.jsx
 ┃ ┃ ┣ 📜ContactForm.jsx
 ┃ ┃ ┣ 📜Footer.jsx
 ┃ ┃ ┣ 📜Hero.jsx
 ┃ ┃ ┣ 📜NavBar.jsx
 ┃ ┃ ┣ 📜ProjectCard.jsx
 ┃ ┃ ┣ 📜Services.jsx
 ┃ ┃ ┣ 📜Skills.jsx
 ┃ ┃ ┗ 📜testimonials.jsx
 ┃ ┣ 📂data
 ┃ ┃ ┣ 📜projects.js
 ┃ ┃ ┣ 📜services.js
 ┃ ┃ ┗ 📜testimonials.js
 ┃ ┣ 📂pages
 ┃ ┃ ┣ 📜AboutMePage.jsx
 ┃ ┃ ┣ 📜ContactPage.jsx
 ┃ ┃ ┣ 📜ErrorPage.jsx
 ┃ ┃ ┗ 📜ProjectsPage.jsx
 ┃ ┣ 📜App.jsx
 ┃ ┣ 📜index.css
 ┃ ┣ 📜main.jsx
 ┃ ┗ 📜routes.jsx
 ┣ 📜.env
 ┣ 📜.gitignore
 ┣ 📜README.md
 ┣ 📜eslint.config.js
 ┣ 📜index.html
 ┣ 📜package-lock.json
 ┣ 📜package.json
 ┣ 📜postcss.config.js
 ┣ 📜tailwind.config.js
 ┣ 📜vercel.json
 ┗ 📜vite.config.js
```

## 📫 Contact

- **Nurudeen Abdul-Majeed**
- **Email**: [abdulmajeednurudeen48@gmail.com]
- **LinkedIn**: [https://www.linkedin.com/in/abdul-majeed-nurudeen-78266a182/]

## 📄 License

© [Nurudeen Abdul-Majeed] All Rights Reserved

**No Permission Granted**

- This project is private and proprietary
- All content, code, and design are exclusive to the owner
- Unauthorized copying, modification, distribution, or use is strictly prohibited
- Written permission required for any use

---

**Built with ❤️ by [Nurudeen Abdul-Majeed]**
