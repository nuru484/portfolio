 ...
 * fields in its `where`. A soft-delete-aware lookup uses `findFirst`
 * instead (see `lib/auth` / `lib/users`).
 */
function withSoftDelete(base: PrismaClient) {
  return base.$extends({
    ...
    query: {
      ...
      clientLogo: {
        delete: ({ args }) =>
          base.clientLogo.update({
            where: args.where,
            data: { deletedAt: new Date() },
          }),
        deleteMany: ({ args }) =>
          base.clientLogo.updateMany({
            where: args.where ?? {},
            data: { deletedAt: new Date() },
          }),
        findMany: ({ args, query }) => {
      ...
      testimonial: {
        delete: ({ args }) =>
          base.testimonial.update({
            where: args.where,
            data: { deletedAt: new Date() },
          }),
        deleteMany: ({ args }) =>
          base.testimonial.updateMany({
            where: args.where ?? {},
            data: { deletedAt: new Date() },
          }),
        findMany: ({ args, query }) => {
