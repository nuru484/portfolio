...
export default async function ProjectDetailPage({ params }: PageProps) {
  ...
  // Cover first, then the screenshots, so the full-view viewer walks the
  // case study in the order the page reads.
  const galleryImages = [
        ...
        <article className="font-urbanist">
          <div className="max-w-4xl mx-auto px-6 md:px-12 pt-6 pb-10">
            ...
            <div className="mb-8 flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <MoveRight aria-hidden strokeWidth={1} className="w-4 h-4" />
              ...
              {isPrivateBuild && (
                <p className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-muted-foreground">
                  <span className="inline-flex items-center gap-1.5">
                    <Lock aria-hidden className="h-4 w-4" /> Private build -
                  </span>
                  <Link
                    href="/contact"
          ...
          <ProjectGallery images={galleryImages} title={project.title}>
            ...
            <div className="max-w-5xl mx-auto px-6 md:px-12">
              <GalleryTile
                ...
                className="aspect-[16/9] rounded-2xl border border-border bg-muted"
              />
            </div>
                  ...
                  <div className="mt-3 grid gap-4 sm:grid-cols-2">
                    {project.screenshots.map((url, i) => (
                      <GalleryTile
                        ...
                        className="aspect-[16/10] rounded-xl border border-border bg-muted"
                      />
                    ))}
                  </div>
                </section>
