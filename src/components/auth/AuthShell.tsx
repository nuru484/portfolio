...
interface AuthShellProps {
  /** Required: it is the page's h1, and every page needs one. */
  title: string;
  subtitle?: string;
...
export function AuthShell({ title, subtitle, children }: AuthShellProps) {
  return (
    <main
      id="main"
      className="min-h-screen flex items-center justify-center px-4 py-12 font-urbanist"
    >
      <div className="w-full max-w-md">
        ...
        <div className="mt-6 flex items-center justify-between gap-3 text-sm text-muted-foreground">
          ...
          >
            <ArrowLeft aria-hidden className="h-3.5 w-3.5" /> Back to site
          </Link>
